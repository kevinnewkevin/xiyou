...
//暂时的 一样 慢慢再改
func CreateUnitFromTable1(id int32) *BattleUnit {
	t := GetUnitRecordById(id)
	if t == nil {
		return nil
	}
...
func CreateBattleUnit(p *GameUnit, b *BattlePlayer) *BattleUnit {
	u := BattleUnit{}
	...
	u.Skill = p.Skill
	u.IsMain = p.IsMain
	u.IProperties = p.IProperties
	u.CProperties = p.CProperties
	u.Owner = b

	return &u
}

func (this *BattleUnit) GetBattleCamp() int {

	return this.Camp
}

func (this *BattleUnit) GetCProperty(id int32) float32 {
	if id <= prpc.CPT_MIN || id >= prpc.CPT_MAX {
		return 0
	}
	return this.CProperties[id]
...
func (this *BattleUnit) GetIProperty(id int32) int32 {
	if id <= prpc.IPT_MIN || id >= prpc.IPT_MAX {
		return 0
	}
	return this.IProperties[id]
...
func (this *BattleUnit) AddSpec(spec string, buffinstid int32) {
	spe := prpc.ToId_BuffSpecial(spec)
	bufflist, ok := this.Special[int32(spe)]
	if !ok {
		this.Special[int32(spe)] = []int32{buffinstid}
...
func (this *BattleUnit) PopSpec(spec string, buffinstid int32) {
	spe := prpc.ToId_BuffSpecial(spec)
	logs.Debug("PopSpec %d %d %d ", buffinstid, spe, this.Special)
...
func (this *BattleUnit) GetSpecial(spec string) []int32 { //获取对应sepce枚举对应的buffid 可能为空
	tmp := []int32{}
	spe := prpc.ToId_BuffSpecial(spec)
	bufflist, ok := this.Special[int32(spe)]
	if !ok {
...
func (this *BattleUnit) GetOneSpecial(spec string, round int32) int32 { //获取对应sepce枚举对应的实力id 可能为空
	var tmp int32
	spe := prpc.ToId_BuffSpecial(spec)
	bufflist, ok := this.Special[int32(spe)]
	if !ok {
...
func (this *BattleUnit) CheckSpec(spec string, round int32) bool { //unit.checkspec(是否有免死)
	spe := prpc.ToId_BuffSpecial(spec)
	bufflist, ok := this.Special[int32(spe)]
...
func (this *BattleUnit) ClacSheldPer(round int32) float32 { //计算百分比减伤 所有buff的百分比减伤加起来 有个最大值
	maxPer := 75
...
func (this *BattleUnit) ClacStrongPer(round int32) float32 { //计算百分比增加输出伤 所有buff的百分比减伤加起来 有个最大值
	maxPer := 75
...
func (this *BattleUnit) ClacWeakPer(round int32) float32 { //计算百分比增加承受伤 所有buff的百分比减伤加起来 有个最大值
	maxPer := 75
...
func (this *BattleUnit) SetUnitCOM(u *prpc.COM_Unit) {
	this.UnitId = u.UnitId
	this.InstId = u.InstId
	this.Level = u.Level
	this.Race = u.Race
...
func (this *BattleUnit) GetUnitCOM() prpc.COM_Unit {
	u := prpc.COM_Unit{}
	u.UnitId = this.UnitId
	u.InstId = this.InstId
	u.Level = this.Level
...
func (this *BattleUnit) GetBattleUnitCOM() prpc.COM_BattleUnit {
	u := prpc.COM_BattleUnit{}
	u.Position = this.Position
	u.InstId = this.InstId
	u.UnitId = this.UnitId
...
func (this *BattleUnit) SelectSkill(round int32) *Skill {
	var idx int32
	if this.MoveStage > 2 {
		idx = this.MoveStage % 3
	} else {
...
func (this *BattleUnit) CastSkill(battle *BattleRoom) bool {
	if this.IsDead() {
		return false
	}
...
func (this *BattleUnit) CastPassiveSkill(battle *BattleRoom) bool {
	skill := this.Skill[0]
...
func (this *BattleUnit) GetFrontList() []int {
	if this.GetBattleCamp() == prpc.CT_RED {
		return []int{prpc.BP_RED_1, prpc.BP_RED_2, prpc.BP_RED_3}
	} else {
		return []int{prpc.BP_BLUE_1, prpc.BP_BLUE_2, prpc.BP_BLUE_3}
...
func (this *BattleUnit) GetBackList() []int {
	if this.GetBattleCamp() == prpc.CT_RED {
		return []int{prpc.BP_RED_4, prpc.BP_RED_5, prpc.BP_RED_6}
	} else {
		return []int{prpc.BP_BLUE_4, prpc.BP_BLUE_5, prpc.BP_BLUE_6}
...
func (this *BattleUnit) IsDead() bool {
	return this.GetCProperty(prpc.CPT_CHP) <= 0
}

func (this *BattleUnit) IsJump() bool {
	buff_lis, ok := this.Special[prpc.BF_JUMP]
	if !ok {
		return false
	}
...
func (this *BattleUnit) isFront() bool {
	li := this.GetFrontList()
...
func (this *BattleUnit) isBack() bool {
	li := this.GetBackList()
...
func (this *BattleUnit) ClearAllbuff() {
...
func (this *BattleUnit) ResetBattle(camp int, ismain bool, battleid int64) {
	this.CProperties[prpc.CPT_HP] = float32(this.IProperties[prpc.IPT_HP])
	this.CProperties[prpc.CPT_CHP] = float32(this.IProperties[prpc.IPT_HP])
	this.Allbuff = []*Buff{}
	this.Camp = camp
...
func (this *BattleUnit) MustUpdateBuff(spe string, round int32) {
	special := prpc.ToId_BuffSpecial(spe)
	bufflist, _ := this.Special[int32(special)]
...
func (this *BattleUnit) SelectBuff(instid int32) *Buff {
	for _, buff := range this.Allbuff {
		if buff.InstId == instid {
			return buff
		}
...
func (this *BattleUnit) CheckAllBuff(round int32) []int32 {
	logs.Debug(string(this.InstId), "checkallBuff round is ", round) //檢測buff效果
	needDelete := map[*Buff]int{}
	this.DelBuff = []*Buff{}
...
func (this *BattleUnit) deletBuff(need map[*Buff]int) []int32 {
	newList := []*Buff{}
	delete_id := []int32{}
	for _, buff := range this.Allbuff {
		_, ok := need[buff]
...
func (this *BattleUnit) PopAllBuffByDebuff() int {
	//删除卡牌身上所有的debuff
	tmp := map[*Buff]int{}
...
func (this *BattleUnit) PopAllBuffByBuff() int {
	//删除卡牌身上的buff
	tmp := map[*Buff]int{}
...
func (this *BattleUnit) ClearBuffByDead(ownerid int64, buffids []int32) {

}

func (this *BattleUnit) PopAllBuffByDead(battle *BattleRoom) {
	if this.UnitId != 19 {
		return
	}
	for _, unit := range battle.Units {
...
func (this *BattleUnit) ChangeBuffTimes(round int32) {
