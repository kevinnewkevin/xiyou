...
func NewCron(tname string, spec string, f TaskFunc) *Cron {
...
func (t *Cron) GetExpr() string {
	return t.Expr
}

// GetStatus get current task status
func (t *Cron) GetStatus() string {
	var str string
...
func (t *Cron) SetExpr(spec string) {
	t.Spec = t.parse(spec)
}

func (t *Cron) parse(spec string) *Schedule {
	...
	if len(fields) != 5 && len(fields) != 6 {
		logs.Panicf("Expected 5 or 6 fields, found %d: %s", len(fields), spec)
	}
...
func (t *Cron) parseSpec(spec string) *Schedule {
	switch spec {
	...
	logs.Panicf("Unrecognized descriptor: %s", spec)
	return nil
}
...
func AddCron(taskname string, t Tasker) {
...
func getRange(expr string, r bounds) uint64 {
	...
	if lowAndHigh[0] == "*" || lowAndHigh[0] == "?" {
		...
		default:
			logs.Panicf("Too many hyphens: %s", expr)
		}
	}
	...
	default:
		logs.Panicf("Too many slashes: %s", expr)
	}

	if start < r.min {
		logs.Panicf("Beginning of range (%d) below minimum (%d): %s", start, r.min, expr)
	}
	if end > r.max {
		logs.Panicf("End of range (%d) above maximum (%d): %s", end, r.max, expr)
	}
	if start > end {
		logs.Panicf("Beginning of range (%d) beyond end of range (%d): %s", start, end, expr)
	}
...
func mustParseInt(expr string) uint {
	...
	if err != nil {
		logs.Panicf("Failed to parse int from %s: %s", expr, err)
	}
	if num < 0 {
		logs.Panicf("Negative number (%d) not allowed: %s", num, expr)
	}
